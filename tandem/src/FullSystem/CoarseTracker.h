...
public:
  ...
  CoarseTracker(int w, int h, bool cuda_coarse_tracking_flag = false);
  ...
  void setCoarseTrackingRef(
      ...
      FrameHessian *ref,
      bool rgbd_init = false,
      TandemCoarseTrackingDepthMap const *dense_depth = nullptr,
      Timer *dr_timer = nullptr
  );
...
//private:
  ...
  void makeCoarseDepthL0(std::vector<FrameHessian *> frameHessians, bool rgbd_init);
  ...
  float *pc_u_nodia;
  float *pc_v_nodia;
  float *pc_idepth_nodia;
  float *pc_color_nodia;
  int pc_n_nodia;
...
#ifdef __AVX__
  AVXAccumulator9<8> acc;
#else
  Accumulator9 acc;
#endif

  std::shared_ptr<dvo::DenseTracker> dense_tracker;

  CudaCoarseTracker *cudaCoarseTracker = nullptr;
};
...
public:
  ...
  void makeK(CalibHessian *HCalib, bool half_pixel = true);
